...
export default async function MatchesPage({
  ...
  searchParams: Promise<{ error?: string; edit?: string; tournament_id?: string; status?: string }>;
}) {
  const { error, edit, tournament_id: tournamentFilter, status: statusFilter } = await searchParams;
  const supabase = await createClient();

  const [{ data: tournaments }, { data: rosters }, { data: scorekeepers }] = await Promise.all([
